...
class TherapeuteController extends AbstractController
{
    ...
    public function index(
        ...
        // 🔍 FONCTIONNALITÉ MÉTIER : Recherche DQL par nom/prénom/spécialité
        $therapeutes = $repo->findBySearch($search, $specialite);

        // 🔍 FONCTIONNALITÉ MÉTIER : Note moyenne + nb avis via DQL
        $noteMoyenne = [];
        $nbAvis      = [];
        foreach ($therapeutes as $t) {
            $noteMoyenne[$t->getId()] = $avisRepo->getNoteMoyenne($t->getId());
            $nbAvis[$t->getId()]      = count($avisRepo->findByTherapeute($t->getId()));
        }

        // 🔍 FONCTIONNALITÉ MÉTIER : Spécialités distinctes via DQL
    ...
    public function adminIndex(
        ...
        // 🔍 FONCTIONNALITÉ MÉTIER : Notes moyennes pour l'admin
        $noteMoyenne = [];
        foreach ($therapeutes as $t) {
            $noteMoyenne[$t->getId()] = $avisRepo->getNoteMoyenne($t->getId());
        }

        return $this->render('therapeute/admin.html.twig', [
            'therapeutes' => $therapeutes,
            'noteMoyenne' => $noteMoyenne,
            'errors'      => [],
        ]);
    ...
    public function new(
        ...
        // ✅ CONTRÔLES DE SAISIE PHP CÔTÉ SERVEUR
        $errors = [];
        ...
        $t->setCertificat($request->request->get('certificat') ?: null);
    ...
    public function edit(
        ...
        // ✅ CONTRÔLE FICHIER PHOTO (modification)
        $imageFile = $request->files->get('image');
        if ($imageFile) {
            $allowedMimes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
            if (!in_array($imageFile->getMimeType(), $allowedMimes)) {
        ...
        $therapeute->setCertificat($request->request->get('certificat') ?: null);
        $em->flush();
